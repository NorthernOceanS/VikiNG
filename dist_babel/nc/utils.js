...
let translator = {
  directionMarkToDirection: function (directionMark) {
    switch (directionMark) {
      case "+x":
        return new _normaCore.Direction(0, -90);

      case "-x":
        return new _normaCore.Direction(0, 90);

      case "+y":
        return new _normaCore.Direction(-90, 0);

      case "-y":
        return new _normaCore.Direction(90, 0);

      case "+z":
        return new _normaCore.Direction(0, 0);

      case "-z":
        return new _normaCore.Direction(0, 180);
...
let utils = {
  ...
  misc: {
    generatePlayerIDFromUniqueID: function (uniqueID) {
      ...
      let high = uniqueID["64bit_high"] % 10000; //hash function:
  ...
  generators: {
    canonical: {
      addFunction: function (type, data, target) {
        ...
        if (indexOfVacancy == -1) utils.logger.log("warning", `Too many ${type}s!New one is ignored`);else {
          target[indexOfVacancy] = data;
          utils.logger.log("info", `New ${type} accepted.`);
        }
      },
      removeFunction: function (index, target) {
        if (index === undefined) for (index = target.length - 1; index >= 0 && target[index] == undefined; index--);
        if (index >= 0) target[index] = undefined;
        utils.logger.logObject("info", target);
      },
      validateParameter: function () {
        let result = new String();
        if (this.blockTypeArray.indexOf(undefined) != -1) result += "Too few blockTypes!Refusing to execute.\n";
        if (this.positionArray.indexOf(undefined) != -1) result += "Too few positions!Refusing to execute.\n";
        if (this.directionArray.indexOf(undefined) != -1) result += "Too few directions!Refusing to execute.";
        if (result == "") result = "success";else utils.logger.log("error", result);
        return result;
      },
      ...
      generatorConstrctor: function ({
        ...
        return new _normaCore.Generator(description, new Array(positionArrayLength).fill(undefined), new Array(blockTypeArrayLength).fill(undefined), new Array(directionArrayLength).fill(undefined), option, function (position) {
          ...
          utils.generators.canonical.postGenerate.call(this);
        }, UIHandler);
      }
    }
  },
  geometry: {
    getDirectionMark: {
      horizontal: function (theta) {
        if (-45 <= theta && theta <= 45) return "+z";else if (-135 <= theta && theta <= -45) return "+x";else if (45 <= theta && theta <= 135) return "-x";else return "-z";
      }
    }
  },
  coordinateGeometry: {
    transform: function (f, g, h) {
      return coordinate => {
        return new _normaCore.Coordinate(f(coordinate.x, coordinate.y, coordinate.z), g(coordinate.x, coordinate.y, coordinate.z), h(coordinate.x, coordinate.y, coordinate.z));
      };
    },
    generateLine: function (x, y, z, t_span, constraint, t_step) {
      ...
      function isRedundant(coordinateArray, newCoordinate) {
        ...
        return coordinateArray[coordinateArray.length - 1].x == newCoordinate.x && coordinateArray[coordinateArray.length - 1].y == newCoordinate.y && coordinateArray[coordinateArray.length - 1].z == newCoordinate.z;
      }
      ...
      for (let t = t_span[0]; t <= t_span[1]; t += t_step) {
        let newCoordinate = new _normaCore.Coordinate(Math.round(x(t)), Math.round(y(t)), Math.round(z(t)));
    ...
    generateLineWithTwoPoints: function (x_start, y_start, z_start, x_end, y_end, z_end) {
      ...
      return this.generateLine(t => {
        return (t - t_span[0]) * x_coefficient + x_start;
      }, t => {
        return (t - t_span[0]) * y_coefficient + y_start;
      }, t => {
        return (t - t_span[0]) * z_coefficient + z_start;
      }, t_span, (x, y, z, t) => {
        return true;
      }, Math.min(x_coefficient == 0 ? t_span[1] - t_span[0] : 1 / x_coefficient, y_coefficient == 0 ? t_span[1] - t_span[0] : 1 / y_coefficient, z_coefficient == 0 ? t_span[1] - t_span[0] : 1 / z_coefficient));
    },
    ...
    generateFilledPlanarTriangle: function (x1, y1, z1, x2, y2, z2, x3, y3, z3) {
      ...
      const G = new _normaCore.Coordinate((x1 + x2 + x3) / 3, (y1 + y2 + y3) / 3, (z1 + z2 + z3) / 3);
      ...
      function signedDistance(x_start, y_start, x_end, y_end) {
        return (x, y) => {
          return (y_end - y_start) * x - (x_end - x_start) * y + x_end * y_start - x_start * y_end;
        };
      }

      return this.generateWithConstraint(x_span, y_span, z_span, (x, y, z) => {
        return Math.abs(A * (x - x1) + B * (y - y1) + C * (z - z1)) < Math.sqrt(A * A + B * B + C * C) / 2 && signedDistance(x1, y1, x2, y2)(x, y) * signedDistance(x1, y1, x2, y2)(G.x, G.y) >= 0 && signedDistance(x1, y1, x3, y3)(x, y) * signedDistance(x1, y1, x3, y3)(G.x, G.y) >= 0 && signedDistance(x2, y2, x3, y3)(x, y) * signedDistance(x2, y2, x3, y3)(G.x, G.y) >= 0 && signedDistance(x1, z1, x2, z2)(x, z) * signedDistance(x1, z1, x2, z2)(G.x, G.z) >= 0 && signedDistance(x1, z1, x3, z3)(x, z) * signedDistance(x1, z1, x3, z3)(G.x, G.z) >= 0 && signedDistance(x2, z2, x3, z3)(x, z) * signedDistance(x2, z2, x3, z3)(G.x, G.z) >= 0 && signedDistance(y1, z1, y2, z2)(y, z) * signedDistance(y1, z1, y2, z2)(G.y, G.z) >= 0 && signedDistance(y1, z1, y3, z3)(y, z) * signedDistance(y1, z1, y3, z3)(G.y, G.z) >= 0 && signedDistance(y2, z2, y3, z3)(y, z) * signedDistance(y2, z2, y3, z3)(G.y, G.z) >= 0;
      });
    },
    generateSphere: function (x, y, z, r) {
      return this.generateWithConstraint([x - r, x + r], [y - r, y + r], [z - r, z + r], (_x, _y, _z) => {
        return (_x - x) * (_x - x) + (_y - y) * (_y - y) + (_z - z) * (_z - z) < r * r;
      });
    },
    generateHollowSphere: function (x, y, z, r) {
      return this.generateWithConstraint([x - r, x + r], [y - r, y + r], [z - r, z + r], (_x, _y, _z) => {
        return (_x - x) * (_x - x) + (_y - y) * (_y - y) + (_z - z) * (_z - z) >= (r - 1) * (r - 1) && (_x - x) * (_x - x) + (_y - y) * (_y - y) + (_z - z) * (_z - z) < r * r;
      });
    },
    generateWithConstraint: function (x_span, y_span, z_span, constraint) {
      ...
      const y_step = 1 / 3;
      const z_step = 1 / 3;
      if (x_span[0] >= x_span[1]) [x_span[0], x_span[1]] = [x_span[1], x_span[0]];
      if (y_span[0] >= y_span[1]) [y_span[0], y_span[1]] = [y_span[1], y_span[0]];
      if (z_span[0] >= z_span[1]) [z_span[0], z_span[1]] = [z_span[1], z_span[0]];

      function verifier(x, y, z) {
        for (let _x = Math.max(x - x_step, x_span[0]); _x <= Math.min(x + x_step, x_span[1]); _x += x_step) for (let _z = Math.max(z - z_step, z_span[0]); _z <= Math.min(z + z_step, z_span[1]); _z += z_step) for (let _y = Math.max(y - y_step, y_span[0]); _y <= Math.min(y + y_step, y_span[1]); _y += y_step) if (constraint(_x, _y, _z)) return true;

        return false;
      }

      for (let x = x_span[0]; x <= x_span[1]; x += 1) for (let z = z_span[0]; z <= z_span[1]; z += 1) for (let y = y_span[0]; y <= y_span[1]; y += 1) if (verifier(x, y, z)) coordinateArray.push(new _normaCore.Coordinate(x, y, z)); // function isRedundant(coordinateArray, newCoordinate) {
      // 	if (coordinateArray.length == 0) return false;
      // 	return (
    ...
    withBresenhamAlgorithm: {
      ...
      generateLineWithTwoPoints: function (x0, y0, z0, x1, y1, z1) {
        ...
        for (;;) {
          coordinateArray.push(new _normaCore.Coordinate(x0, y0, z0));
          if (i-- == 0) break;
          x1 -= dx;
      ...
      generate2DEllipse: function (xm, ym, a, b) {
        ...
        function setPixel(x, y) {
          coordinateArray.push(new _normaCore.Coordinate(x, y, 0));
        }
      ...
      generateFilledPlanarTriangle: function (x1, y1, z1, x2, y2, z2, x3, y3, z3) {
        ...
        generateLine(x2, y2, z2, x3, y3, z3).forEach(({
          x,
          y,
          z
        }) => {
          generateLine(x1, y1, z1, x, y, z).forEach(coordinate => {
            coordinateSet.add(coordinate);
          });
        });
        return coordinateSet.values();
      },
      generateEllipseRect: function (x0, y0, x1, y1) {
  ...
  blockGeometry: {
    getBlockDirection: function (blockType) {
      let directionRelatedBlockStateKey = function () {
        ...
        for (let blockStateKey in blockType.blockState) if (blockDirectionTable[blockStateKey] != undefined) return blockStateKey;

        return "";
      }();

      let directionMap = function () {
        if (blockDirectionTable[directionRelatedBlockStateKey][blockType.blockIdentifier] == undefined) return blockDirectionTable[directionRelatedBlockStateKey]["default"];else return blockDirectionTable[directionRelatedBlockStateKey][blockType.blockIdentifier];
      }();

      let directionMark = function () {
        for (let mark in directionMap) if (directionMap[mark] == blockType.blockState[directionRelatedBlockStateKey]) return mark;
